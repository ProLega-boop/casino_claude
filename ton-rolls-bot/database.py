...
def create_bonus(title: str, description: str, icon: str, reward: float,
                 ...
                 action_url: str, action_label: str) -> int:
    """Create bonus task. Returns new bonus id."""
    with _conn() as db:
        cur = db.execute(
            """INSERT INTO bonuses
               (title,description,icon,reward,bonus_type,channel_username,action_url,action_label)
               VALUES (?,?,?,?,?,?,?,?)""",
            (title, description, icon, reward, bonus_type,
             channel_username, action_url, action_label)
        )
        return cur.lastrowid
