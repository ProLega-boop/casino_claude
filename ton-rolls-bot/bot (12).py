"""bot.py — Telegram bot for RoyalDuel."""
from __future__ import annotations
import logging
from aiogram import Bot, Dispatcher, Router
from aiogram.enums import ParseMode
from aiogram.filters import CommandStart, Command
from aiogram.types import (Message, InlineKeyboardMarkup, InlineKeyboardButton,
                           WebAppInfo, BotCommand,
                           BotCommandScopeAllPrivateChats,
                           BotCommandScopeAllGroupChats,
                           ChatMemberAdministrator, ChatMemberOwner)
import config
import database as db

log = logging.getLogger("bot")
from aiogram.client.default import DefaultBotProperties

bot = Bot(token=config.BOT_TOKEN,
          default=DefaultBotProperties(parse_mode=ParseMode.HTML))
dp     = Dispatcher()
router = Router()
dp.include_router(router)

_bot_id: int | None = None


async def _get_bot_id() -> int:
    global _bot_id
    if _bot_id is None:
        me = await bot.get_me()
        _bot_id = me.id
    return _bot_id


async def setup_commands() -> None:
    """Register bot commands so they appear in Telegram menu."""
    private_cmds = [
        BotCommand(command="start", description="🎮 Запустить RoyalDuel"),
        BotCommand(command="me",    description="📊 Мой профиль и статистика"),
    ]
    group_cmds = [
        BotCommand(command="me", description="📊 Профиль игрока RoyalDuel"),
    ]
    await bot.set_my_commands(private_cmds, scope=BotCommandScopeAllPrivateChats())
    await bot.set_my_commands(group_cmds,   scope=BotCommandScopeAllGroupChats())


def webapp_kb(user_id: int) -> InlineKeyboardMarkup:
    url = f"{config.WEBAPP_URL}?uid={user_id}"
    return InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="⚔️ Открыть RoyalDuel",
                             web_app=WebAppInfo(url=url))
    ]])


@router.message(CommandStart())
async def cmd_start(message: Message):
    uid   = message.from_user.id
    uname = message.from_user.username or ""
    fname = message.from_user.first_name or ""
    args  = message.text.split()
    ref   = None
    if len(args) > 1:
        try:
            r = int(args[1])
            if r != uid:
                ref = r
        except ValueError:
            pass
    if not db.get_user(uid):
        db.upsert_user(uid, uname, fname, referrer_id=ref)
        # Credit tournament score for referrer
        if ref:
            try:
                for t in db.get_active_tournaments():
                    if t["kind"] == "refs":
                        db.add_tournament_score(t["id"], ref, 1)
            except Exception:
                pass
    else:
        db.upsert_user(uid, uname, fname)
    user = db.get_user(uid)
    await message.answer(
        f"⚔️ <b>RoyalDuel</b> — PvP игры на TON!\n\n"
        f"💎 Баланс: <b>{user['balance']:.2f} TON</b>\n\n"
        f"Нажми кнопку и начни играть 👇",
        reply_markup=webapp_kb(uid)
    )


@router.message(Command("me"))
async def cmd_me(message: Message):
    uid = message.from_user.id

    # ── Group chats: reply only when bot is admin ──
    if message.chat.type in ("group", "supergroup"):
        try:
            bot_id = await _get_bot_id()
            member = await bot.get_chat_member(message.chat.id, bot_id)
            if not isinstance(member, (ChatMemberAdministrator, ChatMemberOwner)):
                return
        except Exception:
            return

    user = db.get_user(uid)
    if not user:
        await message.answer("Вы ещё не зарегистрированы. Напишите /start")
        return

    refs      = db.get_referrals(uid)
    ref_count = len(refs)

    # Active tournament position
    tourn_lines = ""
    try:
        for t in db.get_active_tournaments():
            lb    = db.get_tournament_leaderboard(t["id"])
            rank  = next((i + 1 for i, e in enumerate(lb) if e["user_id"] == uid), None)
            score = db.get_user_tournament_score(t["id"], uid)
            unit  = "реф." if t["kind"] == "refs" else "TON"
            if rank:
                score_fmt = f"{int(score)}" if t["kind"] == "refs" else f"{score:.2f}"
                tourn_lines += f"\n🏆 <b>{t['title']}</b>: #{rank} место ({score_fmt} {unit})"
            else:
                tourn_lines += f"\n🏆 <b>{t['title']}</b>: не участвуете"
    except Exception:
        pass

    # Name: show @username if available, otherwise first_name (no @ prefix)
    if user.get("username"):
        name = f"@{user['username']}"
    else:
        name = user.get("first_name") or "Игрок"

    wins      = user.get("games_won", 0)
    played    = user.get("games_played", 0)
    total_won = user.get("total_won", 0.0)
    ref_bal   = user.get("ref_balance", 0.0)
    winrate   = round(wins / played * 100, 1) if played else 0

    text = (
        f"👤 <b>{name}</b>\n"
        f"━━━━━━━━━━━━━━\n"
        f"💎 Баланс: <b>{user['balance']:.2f} TON</b>\n"
        f"💰 Реф. баланс: <b>{ref_bal:.2f} TON</b>\n"
        f"━━━━━━━━━━━━━━\n"
        f"🎮 Игр сыграно: <b>{played}</b>\n"
        f"🏅 Побед: <b>{wins}</b> ({winrate}%)\n"
        f"📈 Всего выиграно: <b>{total_won:.2f} TON</b>\n"
        f"👥 Рефералов: <b>{ref_count}</b>"
        f"{tourn_lines}"
    )

    # Button opens WebApp (works in private AND groups)
    kb = webapp_kb(uid)

    if message.chat.type == "private":
        await message.answer(text, reply_markup=kb,
                             disable_web_page_preview=True)
    else:
        # In groups: send with WebApp button (Telegram allows it)
        await message.answer(text, reply_markup=kb,
                             disable_web_page_preview=True)


# ══════════════════════════════════════════════════════════════
# Withdraw approve / reject callbacks
# ══════════════════════════════════════════════════════════════
import json as _json
from aiogram import F
from aiogram.types import CallbackQuery

@router.callback_query(F.data.startswith("wd_approve:"))
async def cb_wd_approve(call: CallbackQuery):
    """Admin approved withdrawal — notify user."""
    try:
        _, uid_str, amount_str, address = call.data.split(":", 3)
        uid    = int(uid_str)
        amount = float(amount_str)
    except Exception:
        await call.answer("Ошибка данных", show_alert=True)
        return

    if call.from_user.id != config.ADMIN_ID:
        await call.answer("Нет доступа", show_alert=True)
        return

    # Mark as completed in DB
    try:
        db.add_balance_history(uid, "withdraw_approved", 0,
                               note=f"Вывод {amount} TON → {address} APPROVED")
    except Exception:
        pass

    # Notify user
    try:
        await call.bot.send_message(
            uid,
            f"✅ <b>Вывод подтверждён!</b>\n\n"
            f"💎 Сумма: <b>{amount} TON</b>\n"
            f"📬 Адрес: <code>{address}</code>\n\n"
            f"Средства отправлены. Зачисление в сети TON — несколько минут.",
            parse_mode="HTML"
        )
    except Exception as e:
        log.warning(f"Could not notify user {uid}: {e}")

    # Edit admin message
    await call.message.edit_text(
        call.message.text + "\n\n✅ <b>ПОДТВЕРЖДЕНО</b>",
        parse_mode="HTML",
        reply_markup=None
    )
    await call.answer("✅ Вывод подтверждён")


@router.callback_query(F.data.startswith("wd_reject:"))
async def cb_wd_reject(call: CallbackQuery):
    """Admin rejected withdrawal — refund user."""
    try:
        _, uid_str, amount_str, address = call.data.split(":", 3)
        uid    = int(uid_str)
        amount = float(amount_str)
    except Exception:
        await call.answer("Ошибка данных", show_alert=True)
        return

    if call.from_user.id != config.ADMIN_ID:
        await call.answer("Нет доступа", show_alert=True)
        return

    # Refund balance
    try:
        db.update_balance(uid, amount)
        db.add_balance_history(uid, "withdraw_refund", amount,
                               note=f"Вывод {amount} TON ОТКЛОНЁН — возврат")
    except Exception as e:
        log.warning(f"Refund error for uid {uid}: {e}")

    # Notify user
    try:
        await call.bot.send_message(
            uid,
            f"❌ <b>Запрос на вывод отклонён</b>\n\n"
            f"💎 Сумма <b>{amount} TON</b> возвращена на ваш баланс.\n\n"
            f"Если есть вопросы — напишите в поддержку @prolega757.",
            parse_mode="HTML"
        )
    except Exception as e:
        log.warning(f"Could not notify user {uid}: {e}")

    # Edit admin message
    await call.message.edit_text(
        call.message.text + "\n\n❌ <b>ОТКЛОНЕНО — средства возвращены</b>",
        parse_mode="HTML",
        reply_markup=None
    )
    await call.answer("❌ Вывод отклонён, средства возвращены")
