...
async def cmd_me(message: Message):
    ...
    for t in active_tournaments:
        ...
        if rank:
            tourn_lines += f"\n🏆 <b>{t['title']}</b>: #{rank} место ({score:.2f if t['kind']=='won_ton' else int(score)} {kind_unit})"
        else:
    ...
    await message.answer(text, reply_markup=webapp_kb(uid), disable_web_page_preview=True)
