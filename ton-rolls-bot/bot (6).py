...
# Cache bot_id so we don't call get_me() on every group message
...
async def setup_commands() -> None:
    ...
    await bot.set_my_commands(private_cmds,
                              scope=BotCommandScopeAllPrivateChats())
    await bot.set_my_commands(group_cmds,
...
async def cmd_me(message: Message):
    ...
    if message.chat.type in ("group", "supergroup"):
        try:
            ...
            if not isinstance(member, (ChatMemberAdministrator, ChatMemberOwner)):
                return  # stay silent — bot is not admin
        except Exception:
            return
    ...
    for t in active_tournaments:
        ...
        if rank:
            score_fmt = (f"{int(score)}" if t["kind"] == "refs"
                         else f"{score:.2f}")
            tourn_lines += (f"\n🏆 <b>{t['title']}</b>: "
                            f"#{rank} место ({score_fmt} {unit})")
        else:
            tourn_lines += f"\n🏆 <b>{t['title']}</b>: не участвуете"

    name      = (f"@{user['username']}" if user.get("username")
                 else user.get("first_name", "Игрок"))
    ...
    if message.chat.type == "private":
        ...
        # In groups: no inline keyboard, just text
