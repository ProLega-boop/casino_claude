...
async def _notify_tournament_result(tournament_id: int, t: dict | None) -> None:
    """Send Telegram message to each participant about their tournament result."""
    if not t:
        return
    try:
        ...
        prize_places = len(prizes) if prizes else min(3, len(lb))
...
def verify_telegram_init_data(init_data: str, claimed_uid: int) -> tuple[bool, int]:
    ...
    try:
        ...
        # HMAC-SHA256 with key = HMAC-SHA256(key=bot_token, data="WebAppData")
        secret = hmac.new(
        ...
        if not hmac.compare_digest(computed, received_hash):
            log.warning(f"initData HMAC mismatch for uid={claimed_uid} — REJECTING")
            return False, 0  # Reject invalid signature
...
class PvPGame:
    ...
    async def place_bet(self, user_id: int, amount: float,
        ...
        if self.status == "betting" and self.timer <= 1:
            return {"ok": False, "error": "Ставки закрыты — слишком поздно"}
    ...
    async def force_reset(self) -> dict:
        ...
        self.status  = "waiting"
        self.timer   = config.ROUND_DURATION
        # Broadcast refund notice
        await mgr.broadcast({"type": "pvp_reset", "refunded": len(refunded)})
        # Push balance updates to refunded users
        for r in refunded:
...
async def handle(ws: WebSocket, m: dict):
    ...
    # ── Tournaments ──
        ...
        # Active + last 10 finished (for history tab)
        all_t = db.get_all_tournaments()
        result = []
        finished_count = 0
        for t in all_t:
            is_fin = t["status"] == "finished"
            if is_fin:
                if finished_count >= 10: continue
                finished_count += 1
    ...
    elif action == "admin_delete_tournament_history":
        # Hard-delete a finished tournament from history (admin panel)
        uid = int(m.get("user_id", 0))
        if uid != config.ADMIN_ID:
            await mgr.send(ws, {"type": "error", "error": "Forbidden"}); return
        db.delete_tournament_from_history(int(m.get("tournament_id", 0)))
        await mgr.send(ws, {"type": "admin_result", "ok": True,
                             "tournament_action": "history_deleted"})

    elif action == "get_tournament_history":
        # Public tournament history (finished tournaments)
        uid = int(m.get("user_id", 0))
        finished = db.get_finished_tournaments(50)
        result = []
        for t in finished:
            lb = db.get_tournament_leaderboard(t["id"])
            my_score = db.get_user_tournament_score(t["id"], uid)
            my_rank = next((i+1 for i,e in enumerate(lb) if e["user_id"]==uid), None)
            result.append({**t, "leaderboard": lb[:10],
                           "my_score": my_score, "my_rank": my_rank})
        await mgr.send(ws, {"type": "tournament_history", "tournaments": result})
    ...
    # ── Lobby create / join ──
        ...
        # Fetch room before removal to know the bet amount
        room_before = db.get_room(room_id)
        ...
        if removed:
            ...
            user = db.get_user(uid)
            refund_amount = room_before["bet_amount"] if room_before else 0
            await mgr.send(ws, {
                "type": "lobby_leave_result", "ok": True,
                "balance": user["balance"] if user else 0,
                "refund": refund_amount,
            })
            # Send refund notification to the user
            if refund_amount and user:
                await mgr.broadcast_to_user(uid, {
                    "type": "lobby_refund",
        ...
        uid = int(m.get("user_id", 0))
        rooms = db.get_user_rooms(uid)
        # Mark which rooms the user created
        for r in rooms:
            r["created_by_me"] = (r.get("creator_id") == uid)
        await mgr.send(ws, {"type": "my_rooms_update", "rooms": rooms})
    ...
    # ── Admin ──
        ...
        await mgr.send(ws, {"type": "admin_result", "ok": True,
                             "pvp_action": "reset",
                             "refunded": len(result.get("refunded", []))})
        ...
        # Verify by type
        import aiohttp as _ah
        if bonus["bonus_type"] == "channel_boost":
            ...
            try:
                ...
                if len(boosts) < need:
                    await mgr.send(ws, {"type": "bonus_result", "ok": False,
                                         "bonus_id": bonus_id,
                                         "error": f"Нужно {need} буст(а), у вас {len(boosts)}"})
                    return
            except Exception as e:
                log.warning(f"boost check error: {e}")
                await mgr.send(ws, {"type": "bonus_result", "ok": False,
                                     "bonus_id": bonus_id, "error": "Ошибка проверки бустов"})
                return
        elif bonus["bonus_type"] == "channel_sub":
            channel = bonus["channel_username"]
            import aiohttp as _ah
            try:
                async with _ah.ClientSession() as sess:
    ...
    # ── Transfer TON ──
        ...
        if result["ok"]:
            ...
            sender_name = sender.get("username") or sender.get("first_name") or str(uid)
            ...
            # Notify recipient if online — with sender info
    ...
    # ── Admin Profit ──
    ...
    elif action == "admin_set_deposit_lock":
        uid = int(m.get("user_id", 0))
        if uid != config.ADMIN_ID:
            await mgr.send(ws, {"type": "error", "error": "Forbidden"}); return
        ...
        await mgr.send(ws, {"type": "admin_result", "ok": True,
                             "deposit_lock_action": "set", "locked": locked})

    elif action == "admin_get_deposits":
        uid = int(m.get("user_id", 0))
        if uid != config.ADMIN_ID:
            await mgr.send(ws, {"type": "error", "error": "Forbidden"}); return
        entries = db.get_all_deposits(200)
        await mgr.send(ws, {"type": "admin_deposits", "entries": entries})
...
async def deposit_ton(payload: dict):
    """Create a CryptoPay TON invoice and return pay_url."""
    import aiohttp
    ...
    if db.get_setting_str("deposit_locked") == "1":
        raise HTTPException(403, "Пополнение временно отключено администратором")
    user = db.get_user(uid)
    if not user:
    ...
    try:
        ...
        log.error(f"CryptoPay createInvoice error: {e}")
        raise HTTPException(502, "Ошибка платёжного сервиса")

    if not data.get("ok"):
        log.error(f"CryptoPay error: {data}")
        raise HTTPException(502, data.get("error", {}).get("name", "Ошибка"))
...
async def crypto_webhook(payload: dict):
    ...
    # Older format: payload is the full object
    if not isinstance(invoice, dict):
        invoice = payload

    invoice_payload = invoice.get("payload") or payload.get("invoice", {}).get("payload", "")
    amount_str      = invoice.get("amount") or payload.get("invoice", {}).get("amount", "0")
    status          = invoice.get("status") or payload.get("invoice", {}).get("status", "")
    ...
    try:
        ...
        log.warning(f"crypto_webhook: bad payload={invoice_payload} amount={amount_str}")
        return {"ok": True}

    if db.get_setting_str("deposit_locked") == "1":
        log.warning(f"crypto_webhook: deposits locked, ignoring uid={uid} amount={amount}")
        return {"ok": True}

    user = db.get_user(uid)
    if not user:
        log.warning(f"crypto_webhook: user {uid} not found")
        return {"ok": True}

    db.update_balance(uid, amount)
    db.add_balance_history(uid, "deposit", amount, note=f"CryptoPay TON +{amount}")
    log.info(f"crypto_webhook: credited {amount} TON → uid {uid}")

    # Push live balance update if user is online
...
# ══════════════════════════════════════════════════════════════════════════
# Admin endpoints (previously added via patches — now permanent)
# ══════════════════════════════════════════════════════════════════════════

@app.post("/api/admin/pvp-reset")
async def admin_pvp_reset(payload: dict):
    uid = int(payload.get("user_id", 0))
    if uid != config.ADMIN_ID:
        raise HTTPException(403, "Forbidden")
    refunded = db.refund_all_active_bets()
    await pvp_game.force_reset()
    return {"ok": True, "refunded": len(refunded)}


@app.post("/api/admin/deposit-lock")
async def admin_deposit_lock(payload: dict):
    uid    = int(payload.get("user_id", 0))
...
async def wallet_withdraw(payload: dict):
    ...
    if db.get_setting_str("withdraw_locked") == "1":
        raise HTTPException(403, "Вывод временно отключён администратором")
    user = db.get_user(uid)
    if not user:
        raise HTTPException(404, "Пользователь не найден")
    if amount < 0.5:
    ...
    if not (address.startswith("UQ") or address.startswith("EQ")):
        raise HTTPException(400, "Некорректный TON адрес")
    db.update_balance(uid, -amount)
    ...
    try:
        kb = InlineKeyboardMarkup(inline_keyboard=[[
            InlineKeyboardButton(
                text="✅ Подтвердить",
                callback_data=f"wd_approve:{uid}:{amount}:{address}"
            ),
            InlineKeyboardButton(
                text="❌ Отклонить",
                callback_data=f"wd_reject:{uid}:{amount}:{address}"
            ),
        ]])
        await _tgbot.send_message(
            config.ADMIN_ID,
            f"📤 <b>Запрос на вывод</b>\n"
            f"👤 @{uname} (ID: {uid})\n"
            f"💎 Сумма: <b>{amount:.4f} TON</b>\n"
            f"📬 Адрес: <code>{address}</code>\n\n"
            f"Подтвердите и отправьте вручную.",
            parse_mode="HTML",
            reply_markup=kb
        )
    except Exception as e:
        log.warning(f"Could not notify admin about withdrawal: {e}")
...
async def admin_send_gift(payload: dict):
    ...
    # Resolve username → user_id from our DB
    ...
    if not target_user_id:
        raise HTTPException(404, f"Пользователь @{target_username} не найден в боте")
    try:
        async with _ah.ClientSession() as sess:
...
# ── CryptoPay webhook re-enable helper ────────────────────────────────────
@app.post("/api/admin/cryptopay-reactivate")
async def cryptopay_reactivate(payload: dict):
    """Re-enable CryptoPay webhooks after server downtime."""
    import aiohttp as _ah
    uid = int(payload.get("user_id", 0))
    if uid != config.ADMIN_ID:
        raise HTTPException(403, "Forbidden")
    server_url = db.get_setting_str("server_url") or config.SERVER_URL
    webhook_url = server_url.rstrip("/") + "/api/crypto-webhook"
    try:
        async with _ah.ClientSession() as sess:
            async with sess.post(
                ...
                data = await resp.json()
        return {"ok": data.get("ok", False), "webhook_url": webhook_url, "raw": data}
    except Exception as e:
        raise HTTPException(502, str(e))
