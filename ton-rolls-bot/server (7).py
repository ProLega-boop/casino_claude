...
async def _notify_tournament_result(tournament_id: int, t: dict | None) -> None:
    ...
    try:
        ...
        for idx, entry in enumerate(lb):
            ...
            score_str = f"{score} реф." if kind == "refs" else f"{float(score):.2f} TON"
            prize_label = ""
            if idx < len(prizes):
                p = prizes[idx]
...
class PvPGame:
    ...
    async def place_bet(self, user_id: int, amount: float,
        ...
        # Block during spinning OR last 1 second before spin
    ...
    async def force_reset(self) -> dict:
        """Admin: cancel current game, refund all bets, restart fresh."""
        if self._task:
            self._task.cancel()
            self._task = None
        refunded = []
...
async def handle(ws: WebSocket, m: dict):
    ...
    # ── Lobby create / join ──
        ...
        await mgr.send(ws, {"type": "admin_result", "ok": True,
                             "pvp_action": "reset", "refunded": result.get("refunded",[])})
    ...
    # ── Admin ──
        ...
        if bonus["bonus_type"] == "channel_sub":
            ...
            try:
                ...
                log.warning(f"Bonus sub check error: {e}")
                await mgr.send(ws, {"type": "bonus_result", "ok": False,
                                     "bonus_id": bonus_id,
            ...
            try:
                ...
                if count < need:
                    await mgr.send(ws, {"type": "bonus_result", "ok": False,
                                         "bonus_id": bonus_id,
                                         "error": f"Нужно {need} буст(а), у вас {count}. Поднимите канал!"})
                    return
            except Exception as e:
                log.warning(f"Bonus boost check error: {e}")
                await mgr.send(ws, {"type": "bonus_result", "ok": False,
                                     "bonus_id": bonus_id,
