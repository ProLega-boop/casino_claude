...
def get_user_rooms(user_id: int) -> list[dict]:
    """Return all waiting rooms created by this user (public + private)."""
    with _conn() as db:
        rows = db.execute(
...
def create_bonus(title: str, description: str, icon: str, reward: float,
    ...
    with _conn() as db:
        cur = db.execute(
            """INSERT INTO bonuses
               (title,description,icon,reward,bonus_type,channel_username,action_url,action_label,boost_count,repeat_hours)
               VALUES (?,?,?,?,?,?,?,?,?,?)""",
...
def get_user_completed_bonuses(user_id: int) -> list[int]:
    """Return list of bonus_ids that are permanently completed (once_only or cooldown not expired)."""
    from datetime import datetime as _dt, timedelta as _td
    with _conn() as db:
        # Get all bonuses with their repeat_hours
        bonuses = db.execute("SELECT id, repeat_hours FROM bonuses").fetchall()
        result = []
        for b in bonuses:
            bid = b["id"]
            rh  = b["repeat_hours"] or 0
            rows = db.execute(
                "SELECT completed_at FROM bonus_completions WHERE bonus_id=? AND user_id=? "
            ...
            if not rows:
                continue
            if rh == 0:
                # one-time: always completed
                result.append(bid)
            else:
                # repeatable: completed if cooldown not expired
                last = _dt.fromisoformat(rows["completed_at"])
                if (_dt.utcnow() - last).total_seconds() < rh * 3600:
                    result.append(bid)
        return result


def complete_bonus(bonus_id: int, user_id: int, reward: float) -> bool:
    """Mark bonus as completed and credit reward. Returns False if already done (cooldown)."""
    from datetime import datetime as _dt, timedelta as _td
    with _conn() as db:
        bonus = db.execute("SELECT repeat_hours FROM bonuses WHERE id=?", (bonus_id,)).fetchone()
        repeat_hours = (bonus["repeat_hours"] or 0) if bonus else 0
        last = db.execute(
            "SELECT completed_at FROM bonus_completions WHERE bonus_id=? AND user_id=? "
            "ORDER BY completed_at DESC LIMIT 1",
            (bonus_id, user_id)
        ...
        if last:
            if repeat_hours == 0:
                return False  # one-time, already done
            elapsed = (_dt.utcnow() - _dt.fromisoformat(last["completed_at"])).total_seconds()
            if elapsed < repeat_hours * 3600:
...
def delete_tournament(tid: int) -> None:
    """Soft-delete: mark as deleted so it hides from history strip but data is preserved."""
    with _conn() as db:
...
def _migrate_new_tables() -> None:
    ...
    with _conn() as db:
        ...
        try:
            ...
            pass
        # Remove UNIQUE constraint from bonus_completions if it exists
        # (we recreate it without UNIQUE to support repeatable bonuses)
        # Check if we need to migrate
        cols_info = db.execute("PRAGMA table_info(bonus_completions)").fetchall()
