...
def add_ref_balance(referrer_id: int, amount: float, referee_id: int | None = None) -> None:
    """Add to referral pending balance. referee_id tracks which referral generated income."""
    with _conn() as db:
        db.execute(
            "UPDATE users SET ref_balance = ref_balance + ?, "
            "total_ref_earned = total_ref_earned + ? WHERE user_id=?",
            (round(amount, 6), round(amount, 6), referrer_id),
        )
        if referee_id:
            db.execute(
                "INSERT INTO referral_earnings (referrer_id, referee_id, amount) VALUES (?,?,?)",
                (referrer_id, referee_id, round(amount, 6))
            )
        ...
        rows = db.execute(
            """SELECT u.user_id, u.username, u.first_name,
               ...
               LEFT JOIN referral_earnings re
                 ON re.referee_id = u.user_id AND re.referrer_id = ?
               WHERE u.referrer_id = ?
               GROUP BY u.user_id""",
