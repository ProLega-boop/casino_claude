...
def verify_telegram_init_data(init_data: str) -> tuple[bool, int | None]:
    ...
    """
    if not config.VERIFY_INIT_DATA or not config.BOT_TOKEN:
        # Dev mode: skip verification, extract uid from raw string
        try:
            params = parse_qs(unquote(init_data))
            import json as _j
            user_str = params.get("user", ["{}"])[0]
            uid = _j.loads(user_str).get("id")
            return True, int(uid) if uid else None
        except Exception:
            return True, None
...
async def handle(ws: WebSocket, m: dict):
    ...
    if action == "auth":
        ...
        init_data   = m.get("init_data", "")

        # Security: verify Telegram signature
        valid, uid_verified = verify_telegram_init_data(init_data)
        if not valid:
            await mgr.send(ws, {"type": "error", "error": "Unauthorized: invalid initData"})
            return
        if uid_verified and uid_verified != uid_claimed:
            log.warning(f"UID mismatch: claimed={uid_claimed} verified={uid_verified}")
            await mgr.send(ws, {"type": "error", "error": "Unauthorized: uid mismatch"})
            return

        uid  = uid_claimed if not uid_verified else uid_verified
    ...
    # ── Tournaments ──
        ...
        if uid != config.ADMIN_ID:
            await mgr.send(ws, {"type": "error", "error": "Forbidden"}); return
        db.finish_tournament(int(m.get("tournament_id", 0)))
        await mgr.broadcast({"type": "tournament_finished",
