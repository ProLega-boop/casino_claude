...
class PvPGame:
    def __init__(self):
        ...
        self._last_bet: dict[int, float] = {}  # user_id -> timestamp
    ...
    async def place_bet(self, user_id: int, amount: float,
        ...
        now = time.time()
        last = self._last_bet.get(user_id, 0)
        if now - last < 2.0:
            return {"ok": False, "error": "Подождите 2 секунды между ставками"}
        self._last_bet[user_id] = now
...
async def handle(ws: WebSocket, m: dict):
    ...
    if action == "auth":
        ...
        # Security: verify Telegram signature (non-blocking — logs warnings only)
